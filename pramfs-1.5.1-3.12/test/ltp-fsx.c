...
prterr(char *prefix)
{
	prt("%s%s%s(%d)\n", prefix, prefix ? ": " : "", strerror(errno),errno);
}
...
check_eofpage(char *s, unsigned offset, char *p, int size)
{
	...
	for (should_be_zero = last_page + (file_size & page_mask);
		...
		if (*(char *)should_be_zero) {
			prt("Mapped %s: non-zero data past EOF (0x%lx) page offset 0x%x is 0x%04x\n",
...
domapread(unsigned offset, unsigned size)
{
	...
	if ((p = (char *)mmap(0, map_size, PROT_READ, MAP_PRIVATE, fd,
			      (off_t)(offset - pg_offset))) == (char *)-1) {
...
do_preallocate(unsigned offset, unsigned length)
{
	...
	if (syscall(__NR_fallocate, fd, keep_size ? FALLOC_FL_KEEP_SIZE : 0, (loff_t)offset, (loff_t)length) == -1) {
